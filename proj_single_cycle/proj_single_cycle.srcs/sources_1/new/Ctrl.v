...
module Ctrl(
    ...
    always @(*)begin                                            //选择写入寄存器堆的数据
        if(inst_jal | inst_jalr) rf_wsel = 2'b00;
        else if(inst_lui) rf_wsel = 2'b01;
        else if(inst_lb | inst_lbu | inst_lh | inst_lhu | inst_lw) rf_wsel = 2'b10;
        else rf_wsel = 2'b11;
    end
    ...
    always @(*)begin                                            //选择alu的运算方式
        ...
        else if(inst_slt | inst_sltu | inst_slti | inst_sltiu) alu_op = `ALU_SLT;
        else if(inst_beq) alu_op = `ALU_BEQ;
        else if(inst_bne) alu_op = `ALU_BNE;
        else if(inst_blt | inst_bltu) alu_op = `ALU_BLT;
        else alu_op = `ALU_BGE;
